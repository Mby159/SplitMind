...
class ResultAggregator:
    ...
    def aggregate_sequential(
        ...
        aggregated = self.aggregate(
            results, strategy=AggregationStrategy.SEQUENTIAL, context=context
        )
    ...
    def _compare_facts(
        self, id1: str, facts1: List[Dict[str, str]], id2: str, facts2: List[Dict[str, str]]
    ) -> List[ConflictInfo]:
        ...
        for fact1 in facts1:
            ...
            for fact2 in facts2:
                ...
                if (
                    self._similar_context(fact1["context"], fact2["context"])
                    and fact1["value"] != fact2["value"]
                ):
                    conflicts.append(
                        ConflictInfo(
    ...
    def _generate_quality_assessment(self, results: List[SubTaskResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            "excellent_results": sum(
                1 for r in results if r.quality_rating == ResultQuality.EXCELLENT
            ),
    ...
    def _generate_consensus_info(
        self, results: List[SubTaskResult], conflicts: List[ConflictInfo]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "agreement_level": (
                "high" if consensus_score > 0.8 else "medium" if consensus_score > 0.5 else "low"
            ),
        }
    ...
    def _aggregate_sequential(
        ...
        sorted_results = sorted(results, key=lambda r: r.quality_score or 0.5, reverse=True)
    ...
    def _aggregate_voting(
        ...
        consensus_points = [
            kp["original"]
            for kp in sorted(key_points.values(), key=lambda x: x["score"], reverse=True)
    ...
    def _aggregate_best_of(
        ...
        best_result = max(
            results, key=lambda r: (r.quality_score or 0.5, self._score_result(r.result))
        )
    ...
    def _find_common_elements(self, texts: List[str]) -> List[str]:
        ...
        for sentences in all_sentences:
            for sentence in sentences:
                ...
                matches = sum(
                    1
                    for other_sentences in all_sentences
...
Synthesized response:"""
    ...
    def _combine_contents_weighted(self, contents: List[Tuple[str, float]]) -> str:
        ...
        sorted_sentences = sorted(
            unique_sentences.values(), key=lambda x: x["weight"], reverse=True
        )
    ...
    def _extract_unique_info(self, base: str, additional: str) -> Optional[str]:
        ...
        unique = [
            s for s in additional_sentences if self._normalize_sentence(s) not in base_sentences
        ]
    ...
    def _calculate_confidence(
        ...
        confidence = (
            success_rate * 0.3 + avg_quality * 0.4 + provider_diversity * 0.2 - conflict_penalty
        )
