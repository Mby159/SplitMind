...
class PrivacyHandler:
    ...
    def _init_patterns(self) -> Dict[str, re.Pattern]:
        ...
        patterns = {
            ...
            "passport": re.compile(
                r"(?<![A-Z0-9])[EG]\d{8}(?![A-Z0-9])|" r"(?<![A-Z0-9])[A-Z]{1,2}\d{6,9}(?![A-Z0-9])"
            ),
    ...
    def _resolve_overlaps(self, items: List[SensitiveInfo]) -> List[SensitiveInfo]:
        ...
        for item in sorted_items:
            ...
            for existing in result:
                ...
                if not (
                    item.position[1] <= existing.position[0]
                    or item.position[0] >= existing.position[1]
                ):
                    overlap = True
                    ...
                    if risk_priority[item.risk_level] > risk_priority[existing.risk_level] or (
                        item.risk_level == existing.risk_level
    ...
    def redact(self, text: str, strategy: str = "placeholder") -> Tuple[str, Dict[str, str]]:
        ...
        for item in sorted_items:
            ...
            redacted_text = (
                redacted_text[: item.position[0]] + replacement + redacted_text[item.position[1] :]
            )
    ...
    def _generate_recommendations(
        self, items: List[SensitiveInfo], overall_risk: RiskLevel
    ) -> List[str]:
        ...
        if high_items:
            recommendations.append(f"检测到 {len(high_items)} 个高风险信息，建议谨慎处理")
    ...
    def create_data_contract(
        ...
        filtered_items = [item for item in detected if item.info_type in allowed_types]

        blocked_items = [item for item in detected if item.info_type not in allowed_types]
