...
@click.option(
    ...
    default="hybrid",
    help="Execution mode (local_only, hybrid, online)",
)
    ...
    if output_format == "json":
        ...
        if verbose:
            ...
            if result.aggregated_result:
                ...
                console.print(
                    f"[dim]Confidence: {result.aggregated_result.confidence_score:.0%}[/dim]"
                )
...
def preview(task: str, strategy: str):
    ...
    console.print(
        Panel(
            f"Task Type: {preview_result['task_type']}", title="Task Analysis", border_style="blue"
        )
    )
...
def providers():
    ...
    if available_providers:
        ...
        console.print(
            "[yellow]No online providers configured. Add API keys to use online services.[/yellow]"
        )
...
def demo(text: Optional[str], share: bool):
    ...
    """
    from splitmind.demo import PrivacyDemo
...
地址：北京市朝阳区xxx街道123号
    ...
    for info_type, items in result.detected_items.items():
        console.print(
            f"   * {info_type}: {items[0][:30]}..."
            if len(str(items[0])) > 30
            else f"   * {info_type}: {items[0]}"
        )
