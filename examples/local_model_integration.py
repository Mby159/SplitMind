...
Demonstrates how to use local small models with SplitMind Core.
"""
...
def main():
    print("=== SplitMind Local Model Integration Demo ===")
    print("Testing end-side small model integration with SplitMind Core")
    print()
    
    # Configure local model (Ollama)
    local_model_config = LocalModelConfig(
        model_name="llama3.2:3b",  # Small model for fast inference
    ...
    # Create SplitMind Engine with local model
    engine = SplitMindEngine(
        local_model_config=local_model_config
    )
    
    # Test 1: Task analysis with local model
    print("Test 1: Task Analysis with Local Model")
    ...
    if 'local_model_analysis' in analysis:
        print("\nEnhanced analysis from local model:")
    ...
    # Test 3: Privacy protection with local model
    ...
    if privacy_result.privacy_report:
        ...
        if 'enhanced_analysis' in privacy_result.privacy_report:
            ...
            if 'local_pii_detection' in enhanced:
                print("\nEnhanced PII detection from local model:")
                for item in enhanced['local_pii_detection'][:3]:  # Show first 3 items
                    print(f"- {item.get('type')}: {item.get('value')} (confidence: {item.get('confidence', 0):.2f})")
    print()
    
    # Test 4: Preview split with local model
    print("Test 4: Task Split Preview")
